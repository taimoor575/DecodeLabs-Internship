...
Goal:
    Implement a simple, reversible encryption/decryption technique that
    demonstrates the core IPO (Input -> Process -> Output) model of
    cryptographic systems:

        Plaintext (Input) --> Algorithm + Key (Process) --> Ciphertext (Output)

Key Requirements (from project brief):
    ...
    4. Handle edge cases (spaces / punctuation / digits)
    5. Validate that decrypting the ciphertext returns the original plaintext

Author: Taimoor (DecodeLabs Cyber Security Intern - Batch 2026)
"""

# ---------------------------------------------------------------------------
# CORE LOGIC: The Shift
#
# Encryption:  E_n(x) = (x + n) mod 26
# Decryption:  D_n(x) = (x - n) mod 26
#
# Where:
#   x = position of the character in the alphabet (A=0 ... Z=25)
#   n = the key (shift amount)
#
# We use ord()/chr() to move between characters and their ASCII integer
# values, since "we don't shift letters, we shift integers."
# ---------------------------------------------------------------------------
